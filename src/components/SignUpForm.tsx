...
export function SignupForm({ onSwitchToLogin, onSignupSuccess, onBack }: SignupFormProps) {
    ...
    const handleCheckUsername = async () => {
        try {
            const response = await fetch(`http://localhost:8000/auth/check-username/${username}`);
            ...
            alert('아이디 중복 확인 중 오류가 발생했습니다. 백엔드 서버가 실행 중인지 확인해주세요.');
    ...
    const handleSelectAddress = (selectedZipcode: string, selectedAddress: string) => {
    ...
    return (
        <>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} style={{ width: '100%', maxWidth: '500px', maxHeight: '85vh', overflowY: 'auto', padding: '40px', background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(20px)', borderRadius: '20px', boxShadow: '0 8px 32px rgba(45, 139, 95, 0.2)', border: '1px solid rgba(45, 139, 95, 0.1)' }}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <FormField label="우편번호" icon={<MapPin size={20} />}>
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <input type="text" value={zipcode} placeholder="우편번호" readOnly required style={{ flex: 1, padding: '12px 12px 12px 44px', borderRadius: '12px', border: '2px solid rgba(45, 139, 95, 0.2)', fontSize: '14px', boxSizing: 'border-box' }} />
                            <motion.button type="button" onClick={() => setIsAddressModalOpen(true)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} style={{ minWidth: '85px', padding: '12px 16px', height: '46px', borderRadius: '12px', border: 'none', background: 'linear-gradient(135deg, #2D8B5F 0%, #3BA474 100%)', color: 'white', fontSize: '13px', fontWeight: '600', cursor: 'pointer', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>주소찾기</motion.button>
                        </div>
                    </FormField>

                    <FormField label="주소" icon={<MapPin size={20} />}>
                        <input type="text" value={address} placeholder="주소" readOnly required style={{ width: '100%', padding: '12px 12px 12px 44px', borderRadius: '12px', border: '2px solid rgba(45, 139, 95, 0.2)', fontSize: '14px', boxSizing: 'border-box' }} />
                    </FormField>
            ...
            <AddressSearchModal isOpen={isAddressModalOpen} onClose={() => setIsAddressModalOpen(false)} onSelectAddress={handleSelectAddress} />
        </>
    );
}
